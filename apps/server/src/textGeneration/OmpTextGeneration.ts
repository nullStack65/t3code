...
/**
 * Build an Oh My Pi text-generation closure bound to a specific `OmpSettings`
  ...
  const runOmpJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      ...
      const promptResult = yield* Effect.gen(function* () {
        yield* runtime.start();
        // No mode override: omp only has default/plan and the default mode is
        // the right one for one-shot generation. The requested model passes
        // through; when none is requested the CLI's current model wins.
        yield* applyOmpAcpModelSelection({
          ...
          mapError: ({ cause, configId }) =>
            new TextGenerationError({
              operation,
              detail: `Failed to set Oh My Pi ACP config option "${configId}" for text generation.`,
              cause,
            }),
        });
          ...
          Option.match({
            onNone: () =>
              Effect.fail(
                new TextGenerationError({
                  operation,
                  detail: "Oh My Pi request timed out.",
                }),
              ),
